...
export class RulesEngineService {
  ...
  static async validateTaxYear(taxYearId: string): Promise<{
    ...
    let completenessScore = this.calculateCompletenessScore(results);

    // Accountant approval override:
    // If the accountant has manually approved ALL documents, the file is
    // considered complete regardless of what the rules engine found.
    // (Rules engine depends on OCR-extracted docTypes — unreadable docs have none)
    const totalDocs = taxYear.documents.length;
    const approvedDocs = taxYear.documents.filter(
      (d: any) => d.reviewStatus === 'approved'
    ).length;
    if (totalDocs > 0 && approvedDocs === totalDocs) {
      // All docs approved by accountant → minimum score of 90
      completenessScore = Math.max(completenessScore, 90);
    } else if (totalDocs > 0 && approvedDocs > 0) {
      // Some approved → proportional boost (each approved doc = +15 pts, capped at 75)
      const approvalBoost = Math.min(75, Math.round((approvedDocs / totalDocs) * 75));
      completenessScore = Math.max(completenessScore, approvalBoost);
    }
  ...
  private static validateIncomeSources(documents: any[]): ValidationResult[] {
    ...
    const INCOME_DOC_TYPES = [
      'T4', 'T4A', 'T4A_P', 'T4A_OAS', 'T4E', 'T4RIF', 'T4RSP', 'T4FHSA',
      'T5', 'T3', 'T5008', 'T5007', 'T2125',
      'RL1', 'RL2', 'RL3', 'RL5', 'RL6', 'RL8', 'RL10', 'RL31', 'RL32',
      'RECEIPT', 'RRSP',
    ];
    const hasIncomeDoc = docTypes.some((t: string) => INCOME_DOC_TYPES.includes(t));
