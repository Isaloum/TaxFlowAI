    ...
    return res.json({
      ...
      documents: taxYear.documents.map((d) => {
        ...
        return {
          ...
          nameMismatch:         meta.nameMismatch  ?? false,
          extractedDocType:     meta.extractedDocType ?? null,
          expectedYear:         meta.expectedYear  ?? null,
          extractedName:        meta.extractedName ?? null,
          _ownerNameStored:     (d.extractedData as any)?._ownerName ?? null,
          _nameMismatchRaw:     meta.nameMismatch,
          _extractionError:     meta.extractionError ?? null,
        };
      }),
    });
  } catch (error: any) {
    const msg = error?.message || 'Unknown';
    console.error('Get completeness error:', msg, error?.code, error?.meta);
    res.status(500).json({ error: 'Internal server error', _dbError: msg });
  }
};
 ...
 * Client marks their file as ready for accountant review
 */
export const submitForReview = async (req: Request, res: Response) => {
  try {
    ...
    if (taxYear.status === 'submitted' || taxYear.status === 'completed') {
      return res.status(400).json({ error: `Already ${taxYear.status}` });
    }
