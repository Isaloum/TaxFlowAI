...
export class DocumentController {
  ...
  static async presignUpload(req: Request, res: Response) {
    try {
      ...
      const { docType, filename, mimeType, fileSize, docSubtype, ownerName } = req.body;
      ...
      const document = await prisma.document.create({
        data: {
          ...
          // Store ownerName in JSON — no extra DB column needed
          extractedData: ownerName ? { _ownerName: ownerName } : {},
        }
      });
  ...
  static async confirmUpload(req: Request, res: Response) {
    try {
      ...
      try {
        ...
        console.log('[confirm] enqueued to SQS');
      } catch (e: any) {
        console.error('[confirm] SQS enqueue failed:', e?.message);
        // Mark as failed immediately if we can't even enqueue
      ...
      // Non-fatal side-tasks (fire immediately — don't wait for extraction)
      ...
      // Document is now pending — frontend polls until Worker Lambda updates it
