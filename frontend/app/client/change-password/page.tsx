...
export default function ChangePasswordPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Update stored user so isFirstLogin is cleared (role-specific key + legacy key)
      const stored = localStorage.getItem('auth_user_client') || localStorage.getItem('auth_user');
      if (stored) {
        const u = JSON.parse(stored);
        u.isFirstLogin = false;
        localStorage.setItem('auth_user_client', JSON.stringify(u));
